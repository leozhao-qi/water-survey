export const store = async ({ commit, dispatch }, form) => {
    console.log(form)
    let { data } = await axios.post(`${this.urlBase}/api/logbooks`, form)
...
export const show = async ({ commit }, logbookId) => {
    let { data: logbook } = await axios.get(`${this.urlBase}/api/logbooks/${logbookId}`)
...
export const destroy = async ({ dispatch, state }) => {
    let { data } = await axios.delete(`${this.urlBase}/api/logbooks/${state.logbook.id}`)
...
export const destroyFile = async ({ commit, state }, fileId) => {
    let { data } = await axios.delete(`${this.urlBase}/api/files/${fileId}`)
...
export const update = async ({ dispatch, state }, form) => {
    let { data } = await axios.put(`${this.urlBase}/api/logbooks/${state.logbook.id}`, form)
...
export const fetch = async ({ commit, rootState }) => {
    let { data: logbooks } = await axios.get(`${this.urlBase}/api/logbooks`)
...
export const fetchComments = async ({ commit, state }) => {
    let { data: comments } = await axios.get(`${this.urlBase}/api/logbooks/${state.logbook.id}/comments`)
...
export const storeComment = async ({ state, dispatch }, form) => {
    let { data } = await axios.post(`${this.urlBase}/api/logbooks/${state.logbook.id}/comments`, form)
...
export const updateComment = async ({ state, commit }, { commentId, form }) => {
    let { data } = await axios.put(`${this.urlBase}/api/logbooks/${state.logbook.id}/comments/${commentId}`, form)
...
export const destroyComment = async ({ state, commit }, commentId) => {
    let { data } = await axios.delete(`${this.urlBase}/api/logbooks/${state.logbook.id}/comments/${commentId}`)
...
export const fetchLessonPackages = async ({ state, commit }, userId) => {
    let { data: packages } = await axios.get(`${this.urlBase}/api/users/${userId}/packages`)
